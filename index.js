...
async function apply(config, opts = {}) {
  ...
  try {
    ...
    for (const raw of config.repos || []) {
      ...
      if (raw.delete === true) {
        if (!raw.name) throw new Error('repo delete requires a "name"')
...
const REPO_KEYS = new Set([
  ...
  'private', 'internal',
  'defaultBranch', 'wiki', 'projects', 'archived', 'delete', 'init', 'template',
  'merging', 'topics',
  'teams', 'collaborators',
...
function validateConfig(config) {
  ...
  for (const name of Object.keys(config.defaults || {})) {
    ...
    for (const k of Object.keys(entry || {})) {
      if (k === 'delete') {
        console.error('warning: "delete" on defaults "' + name + '" is ignored — set it on the repo entry')
        continue
      }
...
function repoChanged(repo, prev) {
  ...
  if (repo.archived) {
    ...
    // still reconcile it (e.g. deprecation) when the block changed vs state.
...
async function buildRulesetBody(org, ruleset) {
  ...
  if (ruleset.bypassActors) {
    ...
    for (const a of ruleset.bypassActors) {
      let id = a.id || null
      let type = a.type || (a.team ? 'Team' : a.username ? 'User' : a.app ? 'Integration' : null)
      if (a.team) {
      ...
      if (a.app) {
        const app = JSON.parse(await gh(['api', `apps/${a.app}`]))
        id = app.id
        type = 'Integration'
      }
